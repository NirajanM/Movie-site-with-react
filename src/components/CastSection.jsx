        ...
        <div className='relative mt-8 md:mb-10'>
            {!loading ? (
                <>
                    ...
                    ><FaChevronRight /></span>
                    <div className='flex pb-8 md:pb-0 md:mb-8 gap-2 md:gap-6 overflow-x-scroll my-4 no-scrollbar' ref={carouselBox}>
                        {data?.cast?.map((item) => {
                            const profileUrl = item.profile_path ? url.backdrop + item.profile_path : avatar;
                            return (
                                <div
                                    key={item.id}
