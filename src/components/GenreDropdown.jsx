...
export default function GenreDropdown({ mediaType, setExpanded }) {
  ...
  return mediaType === "tv" ? (
    <>
      ...
      <span className="text-sm text-slate-100 pl-1 font-light">
        - {genreTv ? genreTv.name : "All"}
      </span>
      {optionOpen && (
        <div
        ...
        >
          <div className="border-b border-white pb-4 w-full text-center text-sm md:text-md">
            Select Tv Genre :{" "}
          </div>
          {data?.genres.map((opt) => {
            return (
    ...
    <>
      ...
      <span className="text-sm text-slate-100 pl-1 font-light">
        - {genreMovie ? genreMovie.name : "All"}
      </span>
      {optionOpen && (
        <div
          className="absolute top-14 left-0 flex flex-col items-center justify-start text-white w-full bg-black z-50 py-5 gap-5 h-[40vh] overflow-y-scroll border border-gray-600"
